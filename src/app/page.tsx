    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 md:px-12 lg:px-20 py-5"
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full relative flex items-center justify-center" style={{ background: 'radial-gradient(circle, rgba(59,130,246,0.2), transparent)' }}>
            <div className="w-4 h-4 rounded-full border border-[var(--color-brand-primary)]" style={{ boxShadow: '0 0 8px rgba(59,130,246,0.5)' }} />
          </div>
          <span className="font-mono text-sm font-semibold ag-glow-text">DriftGuard</span>
        </div>
        <div className="hidden md:flex items-center gap-8 text-sm text-[var(--color-text-secondary)]">
          ...
          <Link href="/sign-in" className="hover:text-[var(--color-text-primary)] transition-colors">Sign In</Link>
          <Link href="/sign-up" className="ag-button !py-2 !px-5 !text-xs">Get Started →</Link>
        </div>
      </nav>
      ...
      <footer className="relative z-10 border-t border-[var(--color-biolume-border)] px-8 md:px-12 lg:px-20 py-10">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <span className="font-mono text-xs text-[var(--color-text-muted)]">© 2026 DriftGuard. Behavioral intelligence for AI.</span>
