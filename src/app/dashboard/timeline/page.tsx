  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="text-xs text-[var(--color-text-muted)]">{fps.length} version{fps.length !== 1 ? 's' : ''} tracked</span>
      </div>

      {fps.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center" style={{ padding: '5rem 2rem' }}>
          <Clock size={40} className="mx-auto text-[var(--color-text-muted)]" style={{ marginBottom: '1.5rem', opacity: 0.3 }} />
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="ag-card p-6">
                ...
                <div className="space-y-4 text-sm">
                  {[
                    ['Hallucination Score', `${(selected.hallucinationScore * 100).toFixed(1)}%`, selected.hallucinationScore > 0.2 ? '#EF4444' : selected.hallucinationScore > 0.1 ? '#F59E0B' : '#00FF88'],
                    ['Topic Consistency', `${(selected.topicConsistency * 100).toFixed(1)}%`, selected.topicConsistency < 0.75 ? '#EF4444' : selected.topicConsistency < 0.85 ? '#F59E0B' : '#00FF88'],
                    ['Avg Latency', `${selected.avgLatencyMs.toFixed(0)}ms`, selected.avgLatencyMs > 500 ? '#F59E0B' : '#00FF88'],
                    ['Refusal Rate', `${(selected.refusalRate * 100).toFixed(1)}%`, selected.refusalRate > 0.1 ? '#EF4444' : '#00FF88'],
                    ['Sample Count', `${selected.sampleCount}`, '#3B82F6'],
                    ['Formal Tone', `${(selected.toneDistribution.formal * 100).toFixed(0)}%`, '#3B82F6'],
                    ['Technical Tone', `${(selected.toneDistribution.technical * 100).toFixed(0)}%`, '#3B82F6'],
                    ['Empathetic Tone', `${(selected.toneDistribution.empathetic * 100).toFixed(0)}%`, '#3B82F6'],
                  ].map(([k, v, color]) => (
                    <div key={k as string} className="flex justify-between items-center py-1 border-b border-[rgba(59,130,246,0.06)]">
