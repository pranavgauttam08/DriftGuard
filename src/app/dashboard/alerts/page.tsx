  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <span className="text-xs text-[var(--color-muted-text)] font-mono">Filter:</span>
        ...
        <div className="flex gap-2">
          <button onClick={() => setFilterMode('all')}
            ...
            style={{
              padding: '6px 12px',
              background: filterMode === 'all' ? 'rgba(0,255,209,0.1)' : 'transparent',
              border: `1px solid ${filterMode === 'all' ? 'rgba(0,255,209,0.3)' : 'rgba(0,255,209,0.08)'}`,
              color: filterMode === 'all' ? '#00FFD1' : '#5A7A7D',
            }}>All Endpoints</button>
          <button onClick={() => setFilterMode('selected')}
            className="text-xs font-mono rounded-lg transition-all"
            style={{
              padding: '6px 12px',
              background: filterMode === 'selected' ? 'rgba(0,255,209,0.1)' : 'transparent',
              border: `1px solid ${filterMode === 'selected' ? 'rgba(0,255,209,0.3)' : 'rgba(0,255,209,0.08)'}`,
              color: filterMode === 'selected' ? '#00FFD1' : '#5A7A7D',
            }}>Selected Only</button>
        </div>
        {unacked > 0 && (
          <span className="text-xs font-mono" style={{ color: '#FF3D6B' }}>
      ...
      {sorted.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center" style={{ padding: '5rem 2rem' }}>
          <Bell size={40} className="mx-auto text-[var(--color-ghost-text)]" style={{ marginBottom: '1.5rem', opacity: 0.3 }} />
        ...
        <div className="space-y-3">
          {sorted.map((alert, i) => (
            <motion.div key={alert.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className={`bio-card p-5 flex items-start gap-4 ${!alert.acknowledged ? 'border-l-2 border-l-[var(--color-biolume-danger)]' : ''}`}
            >
              <StatusBadge status={alert.severity === 'critical' ? 'critical' : alert.severity === 'warning' ? 'warning' : 'info'} />
              <div className="flex-1 min-w-0">
                <p className="text-sm text-[var(--color-surface-text)]">{alert.message}</p>
                <div className="flex items-center gap-3 mt-2 text-[10px] text-[var(--color-ghost-text)] flex-wrap">
                  <span className="flex items-center gap-1"><Clock size={10} />{timeAgo(alert.timestamp)}</span>
                  <span className="font-semibold" style={{
                    color: dg.endpoints.find(e => e.id === alert.endpointId)?.status === 'critical' ? '#FF3D6B' : '#00FFD1'
              ...
              {!alert.acknowledged && (
                <button onClick={() => dg.acknowledgeAlert(alert.id)} className="bio-button-ghost !py-1.5 !px-4 !text-[10px] flex items-center gap-1 shrink-0">
                  <Check size={12} /> Ack
                </button>
              )}
            </motion.div>
