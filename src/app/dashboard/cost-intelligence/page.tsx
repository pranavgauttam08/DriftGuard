...
export default function CostIntelligencePage() {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {activeTab === 'overview' && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
            {[
              { label: 'Today\'s Cost', value: fmt(metrics.todaysCost), color: '#3B82F6', icon: DollarSign },
              { label: 'Monthly Cost', value: fmt(metrics.monthlyCost), color: '#00E5FF', icon: BarChart3 },
              { label: 'Annual Projected', value: fmt(metrics.annualProjected), color: '#F59E0B', icon: TrendingUp },
              ...
              return (
                <motion.div key={card.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase">{card.label}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="ag-card" style={{ padding: '1.25rem' }}>
              <div className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase mb-3">Budget Utilization</div>
              <div className="flex items-end gap-3">
                <span className="text-2xl font-bold" style={{ color: metrics.budgetUtilization > 80 ? '#EF4444' : metrics.budgetUtilization > 60 ? '#F59E0B' : '#00FF88' }}>
                  {metrics.budgetUtilization.toFixed(0)}%
                </span>
                <span className="text-[10px] text-[var(--color-text-muted)] mb-1">of {fmt(metrics.budget)}/mo</span>
              </div>
              <div className="w-full rounded-full overflow-hidden mt-2" style={{ height: '4px', background: 'rgba(59,130,246,0.06)' }}>
                <div className="h-full rounded-full transition-all" style={{
                  width: `${Math.min(100, metrics.budgetUtilization)}%`,
                  background: metrics.budgetUtilization > 80 ? '#EF4444' : metrics.budgetUtilization > 60 ? '#F59E0B' : '#00FF88',
                }} />
              </div>
            </motion.div>

            {/* Efficiency Score */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="ag-card" style={{ padding: '1.25rem' }}>
              <div className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase mb-3">Cost Efficiency</div>
              <div className="flex items-end gap-3">
                <span className="text-2xl font-bold text-[#00FF88]">{metrics.efficiencyScore}</span>
                <span className="text-[10px] text-[var(--color-text-muted)] mb-1">/100</span>
              </div>
            ...
            {/* Risk Score */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="ag-card" style={{ padding: '1.25rem' }}>
              <div className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase mb-3">Cost Risk</div>
              <div className="flex items-end gap-3">
                <span className="text-2xl font-bold" style={{ color: metrics.riskScore > 60 ? '#EF4444' : metrics.riskScore > 30 ? '#F59E0B' : '#00FF88' }}>
                  {metrics.riskScore}
                </span>
                <span className="text-[10px] text-[var(--color-text-muted)] mb-1">/100</span>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="ag-card" style={{ padding: '1.25rem' }}>
            <div className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase mb-3">Daily Cost (Last 30 Days)</div>
              ...
              {dailyCosts.map((cost, i) => (
                <div key={i} className="flex-1 rounded-t transition-all hover:opacity-80"
                  style={{
                    ...
                    background: cost > metrics.avgDailyCost * 1.5 ? '#EF4444' : cost > metrics.avgDailyCost ? '#F59E0B' : '#3B82F6',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
            {forecasts.map((fc, i) => (
              <motion.div key={fc.period} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className="ag-card" style={{ padding: '1.25rem' }}>
                <div className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase mb-3">
                  {fc.period === '30d' ? '30 Days' : fc.period === '90d' ? '90 Days' : fc.period === '180d' ? '6 Months' : '12 Months'}
                </div>

                <div className="space-y-2">
                  {[
                    { label: 'Expected', value: fmt(fc.expected), color: '#3B82F6' },
                    { label: 'Maximum', value: fmt(fc.maximum), color: '#F59E0B' },
                    { label: 'Worst Case', value: fmt(fc.worstCase), color: '#EF4444' },
                ...
                {/* Mini bar */}
                <div className="mt-3 rounded-full overflow-hidden" style={{ height: '3px', background: 'rgba(59,130,246,0.06)' }}>
                  <div className="h-full rounded-full" style={{ width: `${Math.min(100, (fc.expected / fc.worstCase) * 100)}%`, background: '#3B82F6' }} />
                </div>
              </motion.div>
            ))}
          </div>
              ...
              <thead>
                <tr>
                  {['Model', 'Monthly Cost', 'Cost/Request', 'Quality', 'Latency', 'Risk', 'ROI'].map(h => (
                    <th key={h} className="text-[10px] font-mono text-[var(--color-text-muted)] uppercase text-left"
                      style={{ padding: '8px 12px', borderBottom: '1px solid rgba(59,130,246,0.08)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {comparisons.map(comp => (
                  <tr key={comp.model.id} className="hover:bg-[rgba(59,130,246,0.02)] transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[8px] font-mono rounded-full px-2 py-0.5"
                          style={{
                            ...
                            color: comp.model.tier === 'premium' ? '#EF4444' : comp.model.tier === 'standard' ? '#F59E0B' : '#00FF88',
                          }}>{comp.model.tier}</span>
                        <span className="text-sm font-semibold text-[var(--color-text-primary)]">{comp.model.name}</span>
                      </div>
                    </td>
                    <td className="text-sm font-mono text-[var(--color-text-primary)]" style={{ padding: '12px' }}>{fmt(comp.monthlyCost)}</td>
                    <td className="text-xs font-mono text-[var(--color-text-secondary)]" style={{ padding: '12px' }}>${comp.costPerRequest.toFixed(4)}</td>
                    <td style={{ padding: '12px' }}>
                      <div className="flex items-center gap-2">
                        <div className="rounded-full overflow-hidden" style={{ width: '60px', height: '4px', background: 'rgba(59,130,246,0.06)' }}>
                    ...
                    <td style={{ padding: '12px' }}>
                      <span className="text-xs font-mono" style={{ color: comp.riskScore > 25 ? '#F59E0B' : '#00FF88' }}>{comp.riskScore}%</span>
                    </td>
                    <td className="text-xs font-mono font-semibold text-[#3B82F6]" style={{ padding: '12px' }}>{comp.roi.toFixed(1)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-3">
          {costAlerts.map((alert, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="ag-card flex items-start gap-3" style={{ padding: '1.25rem' }}>
              <AlertTriangle size={16} style={{
                color: alert.severity === 'critical' ? '#EF4444' : alert.severity === 'warning' ? '#F59E0B' : '#00E5FF',
                marginTop: '2px', flexShrink: 0,
              }} />
              <div className="flex-1">
                <span className="text-sm text-[var(--color-text-primary)]">{alert.message}</span>
                <span className="block text-[10px] text-[var(--color-text-muted)] mt-1">{alert.time}</span>
              </div>
              <span className="text-[9px] font-mono rounded-full"
                style={{
                  padding: '2px 8px',
                  background: alert.severity === 'critical' ? 'rgba(255,61,107,0.08)' : alert.severity === 'warning' ? 'rgba(255,184,0,0.08)' : 'rgba(139,92,246,0.08)',
                  color: alert.severity === 'critical' ? '#EF4444' : alert.severity === 'warning' ? '#F59E0B' : '#00E5FF',
                }}>{alert.severity}</span>
            </motion.div>
          ))}
      ...
      {activeTab === 'optimize' && (
        <div className="space-y-4">
          <div className="ag-card flex items-center justify-between" style={{ padding: '1.25rem' }}>
            <div className="flex items-center gap-2">
              <Lightbulb size={16} className="text-[#F59E0B]" />
              <span className="text-sm font-semibold text-[var(--color-text-primary)]">{suggestions.length} Optimization Opportunities</span>
            </div>
          ...
          {suggestions.map((sug, i) => (
            <motion.div key={sug.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-[var(--color-text-secondary)]">{sug.currentModel}</span>
                  <ArrowRight size={14} className="text-[var(--color-text-muted)]" />
                  <span className="text-sm font-semibold text-[#3B82F6]">{sug.suggestedModel}</span>
                </div>
                <span className="text-lg font-bold text-[#00FF88]">-{sug.savingsPercent}%</span>
              </div>

              <p className="text-xs text-[var(--color-text-secondary)] mb-3">{sug.reason}</p>
              ...
              <div className="flex gap-4 text-[10px]">
                ...
                <span className="font-mono rounded-full" style={{ padding: '2px 8px', background: 'rgba(139,92,246,0.06)', color: '#00E5FF' }}>
                  {sug.workloadType}
                </span>
                <span className="font-mono rounded-full" style={{
                  padding: '2px 8px',
                  background: sug.qualityImpact === 'better' ? 'rgba(0,255,136,0.06)' : sug.qualityImpact === 'similar' ? 'rgba(139,92,246,0.06)' : 'rgba(255,184,0,0.06)',
                  color: sug.qualityImpact === 'better' ? '#00FF88' : sug.qualityImpact === 'similar' ? '#00E5FF' : '#F59E0B',
                }}>
