    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="ag-card" style={{ padding: '1.5rem' }}>
        <div className="flex items-center gap-2 mb-3">
          <Book size={18} style={{ color: 'var(--color-brand-primary)' }} />
          <h2 className="text-base font-semibold text-[var(--color-text-primary)]">DriftGuard API v1</h2>
        </div>
        <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed mb-4">
          All endpoints require authentication via Clerk and are scoped by organization. Include your API key in the
          <code className="text-xs px-1.5 py-0.5 rounded mx-1" style={{ background: 'var(--color-biolume-dim)', color: 'var(--color-brand-primary)' }}>Authorization</code>
        ...
        <div className="flex gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md" style={{ background: 'var(--color-biolume-dim)', border: '1px solid var(--color-border)' }}>
            <span className="text-[10px] text-[var(--color-text-muted)]">BASE URL</span>
            <code className="text-xs font-mono text-[var(--color-brand-primary)]">https://your-app.vercel.app/api/v1</code>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md" style={{ background: 'var(--color-biolume-dim)', border: '1px solid var(--color-border)' }}>
            <span className="text-[10px] text-[var(--color-text-muted)]">AUTH</span>
            <code className="text-xs font-mono text-[var(--color-brand-primary)]">Bearer dg_live_...</code>
          </div>
        </div>
      </div>
      ...
      {API_SECTIONS.map(section => {
        ...
        return (
          <div key={section.id} className="ag-card overflow-hidden">
            <button onClick={() => toggleSection(section.id)}
              className="w-full flex items-center justify-between p-4 hover:bg-[rgba(59,130,246,0.02)] transition-colors">
              <div className="flex items-center gap-3">
                <Icon size={16} style={{ color: 'var(--color-brand-primary)' }} />
                <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">{section.title}</h3>
                <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'var(--color-biolume-dim)', color: 'var(--color-text-secondary)' }}>
                ...
                {section.endpoints.map((ep, idx) => {
                  ...
                  return (
                    <div key={epKey} style={{ borderBottom: idx < section.endpoints.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
                      <button onClick={() => toggleEndpoint(epKey)} className="w-full flex items-center gap-3 p-4 hover:bg-[rgba(59,130,246,0.02)] transition-colors text-left">
                        ...
                        <code className="text-xs font-mono text-[var(--color-text-primary)]">{ep.path}</code>
                        <span className="text-xs text-[var(--color-text-secondary)] flex-1">{ep.description}</span>
                        <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(139,92,246,0.1)', color: '#00E5FF' }}>{ep.permission}</span>
                        {isEpExpanded ? <ChevronDown size={12} style={{ color: 'var(--color-text-muted)' }} /> : <ChevronRight size={12} style={{ color: 'var(--color-text-muted)' }} />}
                      </button>
                      ...
                      {isEpExpanded && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pb-4 space-y-3">
                          ...
                          {ep.params && ep.params.length > 0 && (
                            <div>
                              <span className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wider block mb-2">Query Parameters</span>
                              <div className="space-y-1">
                                {ep.params.map(p => (
                                  <div key={p.name} className="flex items-center gap-3 text-xs p-2 rounded" style={{ background: 'rgba(59,130,246,0.02)' }}>
                                    <code className="font-mono text-[var(--color-brand-primary)]" style={{ minWidth: '100px' }}>{p.name}</code>
                                    <span className="text-[var(--color-text-muted)]" style={{ minWidth: '60px' }}>{p.type}</span>
                                    {p.required && <span className="text-[9px] px-1 rounded bg-[#EF444415] text-[#EF4444]">required</span>}
                          ...
                          {ep.bodyFields && ep.bodyFields.length > 0 && (
                            <div>
                              <span className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wider block mb-2">Request Body</span>
                              <div className="space-y-1">
                                {ep.bodyFields.map(f => (
                                  <div key={f.name} className="flex items-center gap-3 text-xs p-2 rounded" style={{ background: 'rgba(59,130,246,0.02)' }}>
                                    <code className="font-mono text-[var(--color-brand-primary)]" style={{ minWidth: '100px' }}>{f.name}</code>
                                    <span className="text-[var(--color-text-muted)]" style={{ minWidth: '80px' }}>{f.type}</span>
                                    {f.required && <span className="text-[9px] px-1 rounded bg-[#EF444415] text-[#EF4444]">required</span>}
                          ...
                          {ep.example && (
                            <div>
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wider">Response Example</span>
                                <button onClick={() => copyToClipboard(ep.example!.response, epKey)} className="flex items-center gap-1 text-[10px] text-[var(--color-text-secondary)] hover:text-[var(--color-brand-primary)] transition-colors">
