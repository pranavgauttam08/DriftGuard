  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-[var(--color-surface-text)]">Adversarial Probe Suite</h2>
          <p className="text-sm text-[var(--color-muted-text)] mt-1">20 probes across 5 categories</p>
        </div>
      ...
      {running && (
        <div className="bio-card p-4">
          <div className="h-2 rounded-full bg-[rgba(0,255,209,0.1)] overflow-hidden">
            <motion.div className="h-full rounded-full bg-[var(--color-biolume-primary)]"
              initial={{ width: '0%' }} animate={{ width: '100%' }}
              transition={{ duration: 20, ease: 'linear' }}
              style={{ boxShadow: '0 0 10px rgba(0,255,209,0.5)' }}
            />
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bio-card p-4 text-center">
          <div className="text-2xl font-bold font-mono text-[var(--color-biolume-tertiary)]">{passCount}</div>
          <div className="text-xs text-[var(--color-muted-text)]">Passed</div>
        </div>
        <div className="bio-card p-4 text-center">
          <div className="text-2xl font-bold font-mono text-[var(--color-biolume-danger)]">{failCount}</div>
          <div className="text-xs text-[var(--color-muted-text)]">Failed</div>
        </div>
        <div className="bio-card p-4 text-center">
          <div className="text-2xl font-bold font-mono text-[var(--color-biolume-danger)]">{criticalCount}</div>
          <div className="text-xs text-[var(--color-muted-text)]">Critical</div>
        </div>
      </div>
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <div key={cat} className="space-y-2">
              <h3 className="text-xs font-mono text-[var(--color-biolume-secondary)] uppercase tracking-wider mb-3">{CAT_LABELS[cat]}</h3>
              {results.map((r, i) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <ShieldCheck size={12} className={r.passed ? 'text-[#00FF88]' : 'text-[#FF3D6B]'} />
                    <StatusBadge status={r.passed ? 'pass' : r.severity === 'critical' ? 'critical' : 'block'} size="sm" label={r.passed ? 'PASS' : 'FAIL'} />
                  </div>
                  <div className="text-[10px] text-[var(--color-muted-text)] leading-relaxed mt-1">{r.explanation}</div>
                  <div className="text-[10px] font-mono text-[var(--color-ghost-text)] mt-1">Score: {r.score.toFixed(2)}</div>
                </motion.div>
              ))}
            </div>
          );
