        ...
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            ...
            style={{
              ...
              color: tab === t.key ? '#3B82F6' : '#5A7A7D',
              background: tab === t.key ? 'rgba(59,130,246,0.06)' : 'transparent',
      ...
      <div style={{ padding: '1.5rem' }}>
        ...
        {tab === 'manual' && (
          <div className="space-y-3">
            <div>
              <label className="block text-xs text-[var(--color-text-secondary)] font-mono mb-1">Query</label>
              <input value={query} onChange={e => setQuery(e.target.value)} placeholder="User's question..." className="bio-input w-full" />
            </div>
            <div>
              <label className="block text-xs text-[var(--color-text-secondary)] font-mono mb-1">Response</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-[var(--color-text-secondary)] font-mono mb-1">Latency (ms)</label>
                <input value={latency} onChange={e => setLatency(e.target.value)} className="bio-input w-full" type="number" />
              </div>
              <div>
                <label className="block text-xs text-[var(--color-text-secondary)] font-mono mb-1">Tokens</label>
        ...
        {tab === 'bulk' && (
          <div className="space-y-3">
            <label className="block text-xs text-[var(--color-text-secondary)] font-mono mb-1">Paste JSON array</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {exampleSets.map((set, i) => (
              <button key={i} onClick={() => loadExampleSet(set)}
                className="text-left rounded-lg transition-all hover:border-[rgba(59,130,246,0.3)]"
                style={{ padding: '14px', background: 'rgba(59,130,246,0.03)', border: '1px solid rgba(59,130,246,0.1)' }}>
                <div className="text-lg" style={{ marginBottom: '4px' }}>{set.icon}</div>
                <div className="text-sm font-semibold text-[var(--color-text-primary)]">{set.name}</div>
                <div className="text-xs text-[var(--color-text-muted)]">{set.description}</div>
              </button>
            ))}
          </div>
        )}
        ...
        {tab === 'apikey' && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-[var(--color-text-secondary)] font-mono mb-2">Your API Key</label>
              <div className="flex items-center gap-2">
                <code className="bio-input flex-1 text-xs font-mono truncate" style={{ padding: '10px 14px' }}>
                  {apiKey || 'Loading...'}
                </code>
                <button onClick={copyKey} className="ag-button-ghost !p-2.5" title="Copy">
                  {copied ? <Check size={14} className="text-[var(--color-pass)]" /> : <Copy size={14} />}
                </button>
              </div>
            </div>
            <div className="flex gap-2">
              {(['curl', 'typescript', 'python'] as const).map(lang => (
                <button key={lang} onClick={() => setCodeLang(lang)}
                  ...
                  style={{
                    padding: '6px 14px',
                    background: codeLang === lang ? 'rgba(59,130,246,0.1)' : 'transparent',
                    border: `1px solid ${codeLang === lang ? 'rgba(59,130,246,0.3)' : 'rgba(59,130,246,0.08)'}`,
                    color: codeLang === lang ? '#3B82F6' : '#5A7A7D',
                  }}>
                  {lang}
                </button>
              ))}
            </div>
            <pre className="text-xs font-mono rounded-lg overflow-x-auto" style={{ padding: '16px', background: 'rgba(0,0,0,0.4)', color: '#8AEACC', border: '1px solid rgba(59,130,246,0.08)', lineHeight: 1.6 }}>
        ...
        {queue.length > 0 && tab !== 'apikey' && (
          <div style={{ marginTop: '1.5rem', borderTop: '1px solid rgba(59,130,246,0.08)', paddingTop: '1.5rem' }}>
            ...
            {/* Progress bar */}
            {queue.length > 0 && (
              <div className="rounded-full overflow-hidden" style={{ height: '4px', background: 'rgba(59,130,246,0.08)', marginBottom: '1rem' }}>
                <motion.div className="h-full rounded-full" style={{ background: 'linear-gradient(90deg, #3B82F6, #00E5FF)' }}
                  animate={{ width: `${(sentCount / queue.length) * 100}%` }} />
              </div>
            )}
            <div className="space-y-1" style={{ maxHeight: '200px', overflowY: 'auto' }}>
              {queue.map((item, i) => (
                <div key={i} className="flex items-center gap-2 text-xs rounded" style={{ padding: '6px 10px', background: item.status === 'sent' ? 'rgba(0,255,136,0.04)' : 'rgba(59,130,246,0.02)' }}>
                  {item.status === 'sent' ? <Check size={12} className="text-[#00FF88] shrink-0" /> :
                   item.status === 'sending' ? <Loader2 size={12} className="animate-spin text-[#3B82F6] shrink-0" /> :
                   <div className="w-3 h-3 rounded-full border border-[rgba(59,130,246,0.2)] shrink-0" />}
                  <span className="truncate text-[var(--color-text-secondary)]">{item.query}</span>
                </div>
              ))}
            </div>
          </div>
