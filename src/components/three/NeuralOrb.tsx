    ...
    <div className="w-[500px] h-[500px] max-w-full relative flex items-center justify-center">
      ...
      {[280, 240, 200, 320].map((s, i) => (
        <div key={i} className="absolute rounded-full border" style={{
          width: s, height: s,
          borderColor: `rgba(0,255,209,${0.1 + i * 0.05})`,
      ...
      {[0, 1, 2].map(i => (
        <svg key={`ring-${i}`} className="absolute" viewBox="0 0 400 400" width="400" height="400"
          ...
          <ellipse cx="200" cy="200" rx={140 + i * 25} ry={60 + i * 10}
            fill="none" stroke={['#00FFD1', '#00E5FF', '#00FF88'][i]} strokeWidth="0.8" opacity="0.3"
      ...
      <div className="relative">
        <div className="w-24 h-24 rounded-full" style={{
          background: 'radial-gradient(circle at 35% 35%, rgba(0,255,209,0.3), rgba(0,229,255,0.15), transparent)',
          boxShadow: '0 0 40px rgba(0,255,209,0.2), 0 0 80px rgba(0,255,209,0.1), inset 0 0 30px rgba(0,255,209,0.1)',
        }} />
        <div className="absolute inset-0 rounded-full border border-[rgba(0,255,209,0.3)]" />
      </div>
      ...
      {Array.from({ length: 20 }).map((_, i) => {
        ...
        return (
          <div key={`dot-${i}`} className="absolute w-1.5 h-1.5 rounded-full" style={{
            ...
            opacity: 0.4 + (i % 4) * 0.1,
            boxShadow: `0 0 6px ${['rgba(0,255,209,0.5)', 'rgba(0,229,255,0.5)', 'rgba(0,255,136,0.5)'][i % 3]}`,
            animationName: 'bio-drift',
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const sphereMat = new THREE.MeshBasicMaterial({ color: '#00FFD1', wireframe: true, transparent: true, opacity: 0.25 });
        const sphere = new THREE.Mesh(sphereGeo, sphereMat);
        scene.add(sphere);

        const dataPoints: InstanceType<typeof THREE.Mesh>[] = [];
        const dpColors = ['#00FFD1', '#00E5FF', '#00FF88'];
        for (let i = 0; i < 120; i++) {
